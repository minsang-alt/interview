...
<details>
<summary><strong style="font-size:1.17em">Redis와 MongoDB의 차이점에 대해 설명해주세요</strong></summary>

```text
Redis와 MongoDB는 각각 캐시성 데이터와 복잡한 문서형 데이터를 처리하는 데 적합하며, 
사용 사례에 따라 선택이 달라집니다.
```
...
```text
Redis가 단일 스레드로 동작함에도 높은 성능을 보이는 핵심 이유는 
I/O 멀티플렉싱 모델을 사용하기 때문입니다.

구체적으로, Redis는 이벤트 루프를 기반으로 동작하며 
epoll과 같은 I/O 멀티플렉싱 메커니즘을 활용합니다. 

클라이언트로부터 여러 요청이 들어오면, 
각 소켓을 논블로킹 모드로 설정하고 비동기적으로 처리합니다.

이때 중요한 점은, I/O 작업이 필요한 요청들을 큐에 담아두고 
OS 커널의 epoll 메커니즘을 통해 
I/O 준비가 완료된 작업들만 효율적으로 처리한다는 것입니다. 

이러한 방식으로 단일 스레드임에도 CPU를 최대한 활용하면서 많은 동시 요청을 처리할 수 있습니다
```
