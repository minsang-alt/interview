...
```text
단점은 복잡성이 좀 증가된다 코드를 이해하기 좀 그렇다 
왜냐하면 동일한 이름의 메서드나 연산자를 쓰기 때문에
```
...
동기는 호출하는 함수가 호출된 함수의 작업완료여부를 신경씁니다. 
반면, 비동기는 호출된 함수만이 작업 완료여부를 신경씁니다.

블로킹과 논블로킹은 호출되는 함수에서 
바로 리턴이 되는 지 안되는 지에 대한 관점입니다.
블로킹은 자신의 작업을 진행하다가 호출된 함수의 작업이 시작되면 
그 작업이 끝날때까지 기다렸다가 끝나서야 자신의 작업을 시작합니다.
논블로킹은 자신의 작업을 진행하다가 호출된 함수의 작업이 시작되자마자 
결과여부 상관없이 바로 리턴이 되고 본인의 작업을 계속하는것을 말합니다.

동기/비동기는 개발자가 직접 코드로 제어할 수 있습니다. 이
는 프로그래밍 모델과 관련이 있으며, 개발자가 코드를 어떻게 구성하고 실행할지 결정합니다.

블로킹과 논블로킹은 주로 I/O 작업과 관련된 OS 수준의 동작을 나타냅니다.
이들은 시스템 호출이나 라이브러리 함수의 특성에 따라 결정되며, 
개발자가 직접적으로 제어하기는 어렵습니다.  
```
...
```text
프로그램은 파일일뿐이었는데 프로그램이 실행되어 메모리 상에 올라오고 
운영체제에 의해 관리되면 이를 프로세스라 부릅니다.

그리고 그 프로세스는 다른 프로세스의 메모리영역에 접근할 수 없습니다.

스레드는 프로세스 내에서 실행되는 작업 단위 입니다. 
한 프로세스 내에서 여러 스레드가 존재할 수 있으며, 
이들은 프로세스가 제공하는 동일한 메모리 공간을 공유합니다.

하지만 각 스레드는 스레드로컬을 보유하고 있는데
이는 서로 공유하지 않기 때문에 비동기처리 시 주의해야 합니다.
```
...
```text
기존 자바 스레드 모델은 Native thread로, 자바의 유저 스레드를 만들면 JNI를 통해 커널 영역을 호출해 OS가 커널 스레드를 생성하고 매핑하여 작업을 수행하는 방식이였습니다.

Virtual Thread는 기존 Java의 스레드 모델과 달리, 플랫폼 스레드와 가상 스레드로 나뉩니다. 
플랫폼 스레드 위에서 여러 Virtual Thread가 번갈아 가며 실행되는 형태로 동작합니다. 마치 커널 스레드와 유저 스레드가 매핑되는 형태랑 비슷합니다.

즉 Virtual Thread는 기존 커널스레드(1) : 유저스레드(1)의 구조가 아닌 커널스레드(1) : 플렛폼스레드(1) : 가상스레드(N)의 구조로 매칭됩니다
```

### Virtual thread는 어떻게 수행되나요?

```text
1. 실행될 virtual thread의 작업을 플랫폼스레드의 workQueue에 push 합니다.
2. Work queue에 있는 작업들은 forkJoinPool에 의해 work stealing 방식으로 여유있는 플랫폼스레드에 의해 처리됩니다.
3. 처리되던 작업들은 I/O나 Sleep으로 인한 interrupt나 작업 완료 시, work queue에서 pop되어 park과정에 의해 그 작업은 다시 힙 메모리로 되돌아갑니다.
4. 그리고 힙에 있던 새로운 작업이 unPark()되어 다른 가상스레드에게 부여하고 다시 플랫폼 스레드와 매핑되어 실행됩니다.
```
...
- 컨텍스트 비용이 저렴합니다. 
가상 스레드는 스케줄링이 가볍고 비용이 적습니다. 가상스레드가 park 과정에서 플랫폼스레드와 해제되고 unpark 과정에서 다시 매핑되기 때문입니다.
...
- 일단 사용하게되면 스레드풀을 지양해야하고, 제한적인 리소스를 접근할 때 세마포어를 활용해야하고,
- synchronized 키워드와 같은 사용으로 인한 Pinning 구간을 지양해야합니다. -> 장점을 해치기 때문입니다 대안으로 ReentrantLock 사용
- 가상 스레드가 아무리 많이 생성되더라도, DB나 톰켓 커넥션 수는 한계가 있고, 서비스에 의존하는 다른 서비스들은 각각의 처리 용량이 존재하기 때문에 병목이 발생해 성능이 급격히 떨어집니다
