...
실제로, 프로젝트에서 초기에는 Task 클래스가 작업 정보 관리, 상태 변경, 담당자 할당 등 모든 기능을 담당했습니다. 
Task, TaskStatus, TaskAssignment 클래스로 분리하여 각 클래스가 단일 책임을 갖도록 리팩토링했습니다.


- 개방폐쇄원칙인 OCP 는 한 클래스의 변경은 최소화하면서, 확장에는 열려있게 해야합니다.
실제로, 새로운 작업 유형 예를들면 버그, 기능 개발등을 추가할 때마다 기존 코드를 수정해야 했습니다.
TaskType 인터페이스를 만들고, 각 작업 유형을 구현 클래스로 만들었습니다.

- 리스코프치환원칙인 LSP 는 서브타입은 언제나 자신의 기반 타입으로 교체할 수 있어야 합니다. 
그리고 하위 클래스의 인스턴스는 상위 클래스의 인스턴스 역할을 수행하는데 문제가 없어야 합니다.
새로운 작업 유형을 추가할 때 기존 코드를 수정하지 않고 새 클래스만 추가하면 되었습니다.
...
```text
equals는 두 객체가 동일한지 비교하고, hashcode는 객체의 메모리 주소를 기반으로 정수값을 생성합니다.
유의할 점은 equals를 동일성과 동등성을 보장받기 위해 재정의를 하게 되는데, 이때 hashcode()도 반드시 재정의 해야합니다.

왜냐하면 만약 equals만 재정의를 했다면 hashcode는 다른 정수값을 반환 합니다. 
이때 equals에서는 동등성을 보장받아 true을 반환하는 데, HashMap이나 HashSet에서 해당 객체들을 저장한다면
...
싱크논블로킹은 제어권이 우리에게 있기 때문에 멀티스레드환경에서 동시에 작업을 처리할 수가 있습니다. 
```
...
하지만 각 스레드는 스레드로컬맵을 보유하고 있는데 이는 서로 공유하지 않기 때문에 비동기처리 시 주의해야 할 부분이 있습니다.
```
...
하지만, 실제로는 HTTP 커넥션 풀이나 JDBC 커넥션 풀, JMS로부터의 요청 등을 고려해야하지만 공식에는 그게 빠져있습니다. 
...
```text
Future 인터페이스는 비동기 작업의 결과를 표현하지만, 결과를 기다릴 때 스레드를 블로킹하고, 여러 Future를 조합하거나 체이닝하는 기능이 부족합니다. 또한, 예외 처리가 번거로워서 유연한 비동기 작업 처리에 한계가 있습니다.

반면에 CompletableFuture는 Future의 기능을 확장한 클래스로, 비동기 작업을 더 유연하게 처리할 수 있습니다. 주요 차이점은:
...
또한, **스레드 풀의 과부하**나 **메모리 부족** 문제를 방지하기 위해 `ThreadPoolExecutor`의 **최대 스레드 수**와 **큐 사이즈**를 제한하는 것을 고려했습니다.
```
...
### 단일책임 원칙을 위배했을때 어떤 문제가 발생하는지? 레이어간 데이터 전달을 optional로 하는것에 대해 어떻게 생각 하는지?
...
```text
네, 주요 정렬 알고리즘에 대해 대략적인 로직, 시간복잡도, 차이점, 특이사항을 중심으로 설명해 드리겠습니다.

1. 버블 정렬
   - 로직: 인접한 두 원소를 비교하여 순서가 잘못되어 있으면 교환
   - 시간복잡도: O(n²)
   - 특이사항: 구현이 매우 간단하지만 대규모 데이터에는 비효율적

2. 선택 정렬
   - 로직: 가장 작은 원소를 찾아 맨 앞으로 이동, 반복
   - 시간복잡도: O(n²)
   - 특이사항: 버블 정렬보다 교환 횟수가 적음

3. 삽입 정렬
   - 로직: 정렬된 부분과 정렬되지 않은 부분으로 나누고, 정렬되지 않은 부분의 원소를 정렬된 부분의 적절한 위치에 삽입
   - 시간복잡도: O(n²), 최선의 경우 O(n)
   - 특이사항: 작은 데이터셋이나 거의 정렬된 데이터에 효율적

4. 퀵 정렬
   - 로직: 피벗을 선택하고 피벗보다 작은 원소와 큰 원소로 분할, 재귀적으로 정렬
   - 시간복잡도: 평균 O(n log n), 최악의 경우 O(n²)
   - 특이사항: 대부분의 경우 매우 빠르며, 많은 실제 구현에서 사용됨
