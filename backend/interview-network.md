...
###  HTTP 헤더의 역할과 중요한 헤더들에 대해 설명해 주세요.
...
### Keep-Alive와 Connection: Close의 차이점
...
### keepalive가 proxy랑 무슨 문제되는거 있어?

```text
HOL(Head-of-Line) 블로킹:

문제: 하나의 Keep-Alive 연결로 여러 요청을 처리할 때, 앞선 요청이 지연되면 뒤의 요청들도 모두 지연됩니다.
프록시 영향: 프록시 서버는 이 문제를 더 악화시킬 수 있습니다. 
클라이언트-프록시, 프록시-서버 간 연결에서 각각 HOL 블로킹이 발생할 수 있기 때문입니다.

Keep-Alive 연결이 유지되면 프록시의 로드 밸런싱 능력이 제한될 수 있습니다.
...
### HttpSession.getAttritube("user") 사용자 A가 접속해도 "user"를 Key로 값을 가져오고,\ 사용자 B가 접속해도 "user"를 Key로 가져옵니다. 같은 Key를 쓰는데 어떻게 A와 B를 구분해서 값을 가져오나요
...
### TCP에서 연결 손실(커넥션 로스)을 감지하는 방법

```text
주요방법으로는 

타임아웃(Time-out): 송신 측은 패킷을 전송할 때마다 해당 패킷의 확인 응답(ACK)을 기다립니다. 
일정 시간이 지나도 ACK가 도착하지 않으면 
송신 측은 해당 패킷이 손실되었거나 네트워크 상에서 연결이 끊겼다고 간주하고 재전송을 시작합니다. 
이 시간을 **Retransmission Timeout (RTO)**라고 하며, 각 패킷마다 RTO 타이머가 설정됩니다.

패킷 재전송(Fast Retransmit): 
 수신 측이 중복된 ACK를 받으면
 이는 패킷이 손실되었음을 나타낼 수 있습니다. 
 TCP는 세 번의 중복된 ACK를 수신하면 해당 패킷이 손실되었음을 추정하고 즉시 손실된 패킷을 재전송합니다. 
 

윈도우 크기 변화: 만약 수신 측의 윈도우 크기가 오랫동안 0으로 유지된다면, 
송신 측은 수신 측이 데이터를 더 이상 처리할 수 없다고 판단하여 연결 문제를 감지할 수 있습니다.
```

### TCP KeepAlive에 대해 아나요?

```text
TCP Keepalive는 활성 상태가 아닌 연결을 감지하고 불필요한 연결을 종료하기 위한 옵션입니다
Keepalive는 주로 장시간 비활성화된 TCP 연결이 여전히 유효한지 확인하기 위해 사용됩니다.

일정 시간 동안(보통 2시간) TCP 연결에 대해 트래픽이 없을 경우,
송신 측은 Keepalive 패킷을 수신 측으로 전송합니다.
수신 측은 이 패킷에 대해 응답해야 하며, 응답이 오면 연결은 여전히 유효하다고 판단합니다. 
만약 수신 측에서 응답이 오지 않으면 송신 측은 연결이 끊겼다고 판단하고 연결을 종료할 수 있습니다.
```
...
### HTTP 프로토콜에서 데이터 압축은 어떻게 이루어지나요?
...
### CORS(Cross-Origin Resource Sharing)가 무엇이며, 어떻게 작동하나요?
